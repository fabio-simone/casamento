          ...
          >
            ...
            {fase === "formulario" && (
              <form onSubmit={pagarPix} className="space-y-3">
                ...
                <button type="submit" disabled={loading || totalItens === 0} className="flex w-full items-center justify-center gap-2 rounded-xl bg-oceano px-6 py-3 text-sm font-semibold uppercase tracking-wide text-offwhite shadow-md transition hover:bg-oceano/90 disabled:opacity-50">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src="/icons/pix.png" alt="" className="h-4 w-4 brightness-0 invert" />
                  {loading ? "Gerando PIX…" : `Pagar com PIX — ${formatBRL(totalValor)}`}
                </button>
                <button type="button" disabled={loading} onClick={pagarCartao} className="flex w-full items-center justify-center gap-2 rounded-xl bg-oceano/15 px-6 py-3 text-sm font-semibold uppercase tracking-wide text-oceano transition hover:bg-oceano/25 disabled:opacity-50">
